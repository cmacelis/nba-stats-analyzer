...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // When _subpath comes from query param, Vercel already decodes it.
    // Only decodeURIComponent when parsing from URL path (fallback).
    const playerName = req.query._subpath ? subpath : decodeURIComponent(subpath);
    return await researchHandler(req, res, playerName);
  } catch (err) {
