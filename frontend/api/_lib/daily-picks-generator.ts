/**
 * Daily picks generator — library helper (NOT a serverless endpoint).
 *
 * Generates daily picks using live edge data and produces a Discord-ready
 * text message. Uses the play_score / play_tier system and
 * the formatDailyPicksForDiscord() formatter.
 *
 * Can be called from discord-today.ts or any other endpoint.
...
/**
 * Generate and post daily picks to Discord webhook.
 * Returns the result including the message that was posted.
 */
export async function postDailyPicksToDiscord(
  stat: StatKey = 'pts',
  minMinutes = 20,
  season: number = BDL_SEASON,
): Promise<{ ok: boolean; message: string; aTier: number; bTier: number; totalProcessed: number }> {
