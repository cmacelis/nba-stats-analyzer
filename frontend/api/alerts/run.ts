/**
 * POST /api/alerts/run?stat=pts|pra&min_minutes=20&min_delta=3.0&top_n=10&season=2025
 *
 * Runs the edge feed, finds players above the delta threshold (positive delta = over signal),
 * deduplicates via KV cooldown, and posts Discord embeds for newly-triggered edges.
 *
 * Required env vars:
 *   DISCORD_WEBHOOK_URL     — Discord webhook URL (Server Settings → Integrations → Webhooks)
 *
 * Optional env vars (have sensible defaults):
 *   ALERT_MIN_DELTA_PTS     — minimum L5-vs-season delta for PTS alerts (default: 3.0)
 *   ALERT_MIN_DELTA_PRA     — minimum L5-vs-season delta for PRA alerts (default: 4.5)
 *   ALERT_COOLDOWN_MINUTES  — cooldown per player+stat before re-alerting (default: 180)
 ...
 *   Redis key: nba:alert:cd:{stat}:{player_id}  TTL = ALERT_COOLDOWN_MINUTES * 60
 *   If KV is not configured, cooldown is skipped (every run sends all matching alerts).
 *
 * Scheduling (recommended):
 *   Add a Vercel Cron job in vercel.json:
 *     { "crons": [{ "path": "/api/alerts/run?stat=pts", "schedule": "0 * * * *" }] }
 *   Or trigger manually / from an external scheduler.
 */
...
const MIN_DELTA_PTS = parseFloat(process.env.ALERT_MIN_DELTA_PTS  || '3.0');
const MIN_DELTA_PRA = parseFloat(process.env.ALERT_MIN_DELTA_PRA  || '4.5');
...
const cdKey = (stat: string, playerId: number) => `nba:alert:cd:${stat}:${playerId}`;
...
async function onCooldown(stat: string, playerId: number): Promise<boolean> {
  if (!KV_OK) return false;
  const val = await kvCmd('GET', cdKey(stat, playerId));
  return val !== null;
}

/** Sets the cooldown key with TTL. Called after a successful Discord post. */
async function markAlerted(stat: string, playerId: number): Promise<void> {
  if (!KV_OK) return;
  await kvCmd('SET', cdKey(stat, playerId), '1', 'EX', COOLDOWN_SECS);
}
...
function buildEmbed(entry: EdgeEntry, stat: StatKey, minMinutes: number, season: number): object {
  const label  = stat === 'pra' ? 'PRA' : 'PTS';
  const sign   = entry.delta >= 0 ? '+' : '';
  const url    = compareUrl(entry, season);

  return {
    title:       `🔥 ${entry.player_name} (${entry.team_abbrev}) — ${label} Over Edge`,
    color:       0x22c55e, // green
    description: `**${sign}${entry.delta.toFixed(1)}** above season average`,
    fields: [
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Only over-signals (positive delta) above threshold, capped at top_n
    const candidates = entries.filter(e => e.delta >= minDelta).slice(0, topN);
    ...
    for (const entry of candidates) {
      if (await onCooldown(stat, entry.player_id)) {
    ...
    if (toSend.length > 0) {
      const embeds = toSend.map(e => buildEmbed(e, stat, minMinutes, season));
    ...
    await Promise.all(toSend.map(e => markAlerted(stat, e.player_id).catch(() => null)));

    return res.json({
      ok:               true,
      sent:             toSend.map(e => e.player_name),
      skipped,
